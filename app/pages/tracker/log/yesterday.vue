...
// yesterday
const { data: yesterdayResponse } = await sanctumFetch(
  `/api/trackers/date?date=${yesterdayDate}`
);
const yesterdayTrackers = reactive<TrackerDate[]>(yesterdayResponse);
...
<template>
  ...
  <div class="w-full md:max-w-[600px] mx-auto">
    <!-- Card Loop -->
    <UCard
      v-for="tracker in yesterdayTrackers"
      :key="tracker.order"
      class="mb-5"
    >
      <template #header>
    ...
    <!-- Save Button -->
    <UButton @click="save(yesterdayTrackers, yesterdayDate)" block size="lg"
      >Save</UButton
    >
  </div>
</template>
